...
if handles.hasAnnoFile
    if ~exist(bwFile,'file')
        error('bwFile doesnot exist!');
    end
	bw=imread(bwFile);
	bw=(bw~=0);
...
% If there exists a good soma image.
if strcmp(reply,'y')
	files=getImgFileNames;
	if files{1}==0
		return;
...
% If there is no soma image, manual operation on the bw image is used to
% obtain somabw image.
else
	fH=handles.fH;
	figure(fH);
